...
export const autoLoginService = {
  startAutoLogin: async (request: AutoLoginRequest): Promise<AutoLoginResponse> => {
    try {
      const response = await apiClient.post(API_CONFIG.ENDPOINTS.ADMIN.AUTO_LOGIN, request);
  ...
  getLoginStatus: async (sessionId: string): Promise<LoginStatus> => {
    try {
      const response = await apiClient.get(`${API_CONFIG.ENDPOINTS.ADMIN.AUTO_LOGIN_STATUS}/${sessionId}`);
      return response.data;
    } catch (error) {
  ...
  stopAutoLogin: async (sessionId: string): Promise<AutoLoginResponse> => {
    try {
      const response = await apiClient.post(`${API_CONFIG.ENDPOINTS.ADMIN.AUTO_LOGIN_STOP}/${sessionId}`);
      return response.data;
    } catch (error) {
  ...
  submitOTP: async (sessionId: string, otp: string): Promise<AutoLoginResponse> => {
    try {
      const response = await apiClient.post(`${API_CONFIG.ENDPOINTS.ADMIN.AUTO_LOGIN_OTP}/${sessionId}`, { otp });
      return response.data;
    } catch (error) {
  ...
  submitCaptcha: async (sessionId: string, captchaSolution: string): Promise<AutoLoginResponse> => {
    try {
      const response = await apiClient.post(`${API_CONFIG.ENDPOINTS.ADMIN.AUTO_LOGIN_CAPTCHA}/${sessionId}`, { captchaSolution });
      return response.data;
    } catch (error) {
