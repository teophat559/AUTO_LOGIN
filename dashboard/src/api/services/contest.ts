...
export const contestService = {
  ...
  getContestDetails: async (id: string): Promise<Contest> => {
    return apiClient.get<Contest>(API_ENDPOINTS.CONTESTS.DETAILS.replace(':id', id));
  },
  ...
  submitContest: async (data: SubmitContestRequest): Promise<ContestSubmission> => {
    ...
    return apiClient.post<ContestSubmission>(
      API_ENDPOINTS.CONTESTS.SUBMISSIONS.replace(':id', data.contestId),
      formData,
      {
        headers: {
  ...
  getMySubmissions: async (contestId: string): Promise<ContestSubmission[]> => {
    return apiClient.get<ContestSubmission[]>(API_ENDPOINTS.CONTESTS.SUBMISSIONS.replace(':id', contestId));
  },
};